...
6. Так же проверка роли пользователя осуществляется на `frontend`. Если прав на доступ к отчету нет - кнопка выгрузки отчета получает аттрибут `disabled`
   - в целях тестирования сейчас к кнопке применены только стили - чтобы показать, что на frontend тоже надо реагировать на роли
   - сама возможность отправить запрос на бэк нажав кнопку оставлена - в ответ должна вернуться ошибка 
